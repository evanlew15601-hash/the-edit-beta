...
export function CharacterCreation({
  ...
  const isAgeInvalid = age !== '' && (typeof age !== 'number' || age < 18 || age > 65);

  const handleSubmit = () => {
    if (!canSubmit) return;
    const id = `player_${name.toLowerCase().replace(/\s+/g, '_')}`;
    const persona =
      statBiased.strategy >= statBiased.social &&
      statBiased.strategy >= statBiased.physical &&
      statBiased.strategy >= statBiased.deception
        ? 'Strategic Player'
        : statBiased.social >= statBiased.physical &&
          statBiased.social >= statBiased.deception
          ? 'Social Butterfly'
        ...
        <Card className="p-6 space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">
              Simple setup: choose a background and where you excel.
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Episode 0: Set your persona before the season begins.
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {STAT_OPTIONS.map(opt => (
                <Button
                  ...
                  className="h-auto py-3 min-h-[72px]"
                >
                  <div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <Button
                variant={specialKind === 'none' ? 'action' : 'surveillance'}
                onClick={() => setSpecialKind('none')}
              >
                None
              </Button>
              <Button
          ...
          <div>
            ...
            {!canSubmit && (
              <p className="mt-2 text-xs text-muted-foreground">
                Enter a name (min 2 characters) and an age between 18 and 65 to start your season.
              </p>
            )}
          </div>
        </Card>
